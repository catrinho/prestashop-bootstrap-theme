...
<div id="new-products_block_right" class="block products_block">
	...
	{if $new_products !== false}
		...
		{foreach from=$new_products item=newproduct name=myLoop}
			...
			{if $newproduct.description_short}<dd class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}"><a href="{$newproduct.link}">{$newproduct.description_short|strip_tags:'UTF-8'|truncate:75:'...'}<span class="badge badge-info pull-right">{l s='Read more' mod='blocknewproducts'}</span></a></dd>{/if}
		{/foreach}
		</dl>
		<p><a href="{$link->getPageLink('new-products')}" title="{l s='All new products' mod='blocknewproducts'}" class="btn btn-primary btn-mini">{l s='All new products' mod='blocknewproducts'}</a></p>
	{else}
